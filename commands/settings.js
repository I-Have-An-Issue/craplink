...
exports.run = async (client, message, args) => {
    ...
    switch (args[0]) {
        case "list":
            let fields = []
            for (let i = 0; i < settings.length; i++) { fields.push({ name: settings[i].name, value: settings[i].description }) }
        ...
        case "set":
            ...
            await database.write("settings", { _id: message.guildID }, { [args[1]]: args[2] })
        ...
        case "reset":
            server = await database.read("settings", { _id: message.guildID })
            if (!settings.find(setting => setting.name == args[1])) return message.channel.createMessage(constants.embed_SettingsUnknownSetting(message))
            database.write("settings", { _id: message.guildID }, { [args[1]]: settings.find(setting => setting.name == args[1]).default })
            .then(() =>  message.channel.addMessageReaction(message.id, "✅"))
            .catch(e => {
                message.channel.addMessageReaction(message.id, "❌")
                console.log(e)
